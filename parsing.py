...
def get_copa_price(item, headers):
    link = 'https://www.copa.com.ua//index.php?route=product/search&search=' + item
...
def get_hunter_price(item, headers):
    link = 'https://sport-hunter.com.ua/uk/index.php?route=product/search&search=' + item
    response = requests.get(link, headers=headers).text
    soup = BeautifulSoup(response, 'lxml')
...
def get_korzina_2_price(item, headers):
    link = 'https://football-world.com.ua/ru/search?search=' + item
    response = requests.get(link, headers=headers).text
    soup = BeautifulSoup(response, 'lxml')
...
def get_playfootball_price(item, headers):
    link = 'https://playfootball.com.ua/shop/search?text=' + item
    response = requests.get(link, headers=headers).text
    soup = BeautifulSoup(response, 'lxml')
...
def get_soccer_shop_price(item, headers):
    link = 'https://soccer-shop.com.ua/ua/advanced_search_result.php?keywords=' + item
    response = requests.get(link, headers=headers).text
    soup = BeautifulSoup(response, 'lxml')
